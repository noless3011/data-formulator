            ...
            return [
                ...
                ...templates.map((t, i) => (
                    <MenuItem sx={{ fontSize: 12, paddingLeft: 3, paddingRight: 3 }}
                        ...
                        onClick={(e) => {
                            dispatch(dfActions.createNewChart({ tableId: tableId, chartType: t.chart }));
                            setAnchorEl(null);
                            e.stopPropagation();
                        }}>
                        <ListItemIcon>
...
export const ChartEditorFC: FC<{
    cachedCandidates: DictTable[],
                        ...
                        <Collapse in={codeViewOpen}>
                            <Box sx={{ minWidth: 440, maxWidth: 800, padding: "0px 8px", position: 'relative', margin: '8px auto' }}>
                                ...
                                <Card variant="outlined" key={`code-view-card`}
                                    sx={{
                                        minWidth: "280px", maxWidth: "1920px", display: "flex", flexGrow: 1,
                        ...
                        <Collapse in={codeExplViewOpen}>
                            <Box sx={{ minWidth: 440, maxWidth: 800, padding: "0px 8px", position: 'relative', margin: '8px auto' }}>
                                ...
                                <Card variant="outlined" key={`code-explanation`}
                                    sx={{
                                        minWidth: "280px", maxWidth: "1920px", display: "flex", flexGrow: 1, margin: "0px",
                                        border: "1px solid rgba(33, 33, 33, 0.1)"
                                    }}>
                                    <CardContent sx={{ display: "flex", flexDirection: "column", flexGrow: 1, padding: '0', paddingBottom: '0px !important' }}>
                                        <Typography sx={{ fontSize: 14, margin: 1 }} gutterBottom>
...
export const VisualizationViewFC: FC<VisPanelProps> = function VisualizationView({ }) {
    ...
    const conceptShelfItems = useSelector((state: DataFormulatorState) => state.conceptShelfItems);
    const [collapseDatabaseEditor, setCollapseDatabaseEditor] = useState<boolean>(true);
    const dispatch = useDispatch();
        ...
        let chartSelectionBox = <Box sx={{ display: "flex", flexDirection: "row", width: '720px', flexWrap: "wrap" }}>
            {Object.entries(CHART_TEMPLATES).map(([cls, templates]) => templates).flat().filter(t => t.template["name"] != "?").map(t =>
                <Button
                    key={`${t.chart}-btn`}
                    sx={{
                        margin: '2px', padding: '2px', display: 'flex', flexDirection: 'column',
        ...
        let chartElements = charts.filter(c => !c.intermediate).map((chart, index) => {
            ...
            if (chart.chartType == "Table") {
                return <Box key={`animateOnChange-${index}`}
                    ...
                    sx={{
                        position: 'relative', backgroundColor: chart.saved ? "rgba(255,215,0,0.05)" : "white",
    ...
    } else if (visViewMode == "database") {


        finalView = (
            <Box sx={{ width: "100%", overflow: "hidden", display: "flex", flexDirection: "row-reverse" }}>
                <Box className="visualization-carousel" sx={{ display: "contents" }} >
                    <Collapse
                        key='encoding-shelf'
                        collapsedSize={48} in={!collapseDatabaseEditor} orientation='horizontal'
                        sx={{ position: 'relative' }}>
                        <Box sx={{ display: 'flex', flexDirection: 'row', height: '100%' }}>
                            <Tooltip placement="left" title={collapseDatabaseEditor ? "open editor" : "hide editor"}>
                                <Button color="primary"
                                    sx={{ width: 24, minWidth: 24 }}
                                    onClick={() => { setCollapseDatabaseEditor(!collapseDatabaseEditor) }}
                                >{collapseDatabaseEditor ? <ChevronLeftIcon /> : <ChevronRightIcon />}</Button>
                            </Tooltip>
                            <Box className="test-box" sx={{ width: "200px", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                                <Box sx={{ height: "fit", width: "90%", border: "1px solid rgba(0, 0, 0, 0.12)" }}>
                                    <TextField hiddenLabel id="database-field" placeholder=" host" variant="filled" />
                                    <TextField hiddenLabel id="database-field" placeholder="port" variant="filled" />
                                    <TextField hiddenLabel id="database-field" placeholder="database" variant="filled" />
                                    <TextField hiddenLabel id="database-field" placeholder="db user" variant="filled" />
                                    <TextField hiddenLabel id="database-field" placeholder="db password" variant="filled" type="password" />
                                </Box>
